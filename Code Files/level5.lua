...
function level5:init()
	...
	self:addChild(wall.new(self.world, 390,250, 80, 5, wallColour, 0))
	self:addChild(ice.new(self.world, 390, 250, .4,.7,.35))
	self:addChild(wall.new(self.world, 250,150, 80, 5, wallColour, 0))
	
	-- Wood
	self:addChild(wood.new(self.world, 250, 100))
	...
	-- Orbs 
	self:addChild(orb.new(self.world,390, 185, "images/yelloworb.png", 20))
	self:addChild(orb.new(self.world,250, 50, "images/orb.png", 10))
	self:addChild(orb.new(self.world,250,screenH - 100, "images/redorb.png", 30))
	
	-- bricks 
	self:addChild(brick.new(self.world,250, screenH - 10))
	self:addChild(brick.new(self.world,250, screenH - 30))
	...
	self.angMess2 = {}
	for i = 1, 2 do 
		self.angMess2[1] = TextField.new(nil, "Power") 
		self.angMess2[2] = TextField.new(nil, "Power") 
...
function level5:onEnterFrame() 
	...
	for i = 1, self:getNumChildren() do
		...
		if sprite.body then
			...
			if self.points == 60 and conf.onLevel == 5 then 	
				conf.onLevel = 5 -- 6th level has'nt been made 
...
function level5:onBeginContact(event)
	...
	if (bodyA.type == "ball" and bodyB.type == "wood") or (bodyB.type == "ball" and bodyA.type == "wood") then 
		if bodyA.type == "wood" then 
			if bodyA.scaleFactor < 1.5 then 
				bodyA.scaleFactor = bodyA.scaleFactor * 1.08
				bodyA.changeScale = true 
			end
		else	
			if bodyB.scaleFactor < 2 then 
				bodyB.scaleFactor = bodyB.scaleFactor * 1.08
	...
	if bodyA.type == "sponge" and bodyB.type == "ball" or bodyA.type == "ball" and bodyB.type == "sponge" then 
		print("Jump High")
	end 
	
end 
	
 function level5:onExitBegin()
...
function level5:msUp(event)
	...
	if self.updownBtn:hitTestPoint(x, y) then 
		if y > self.updownBtn:getY() + (self.updownBtn:getHeight() / 2) then
			self.xV = self.xV - 5 
		...
		elseif y < self.updownBtn:getY() + (self.updownBtn:getHeight() / 2) then 
